//
// Created by bob on 7/2/21.
//

#ifndef BODYWORK_PLY_READER_H
#define BODYWORK_PLY_READER_H
...
void read_ply_file(const std::string& filepath, std::vector<float3>& myVerts, std::vector<Color>& myColors,
	const bool preload_into_memory = true)
{
	...
	try
	{
		...
		std::shared_ptr<PlyData> vertices, normals, colors, colorsRGB, texcoords, faces, tripstrip;
		...
		try
		{
			colorsRGB = file.request_properties_from_element("vertex", { "red", "green", "blue" });
		}
		catch (const std::exception& e)
		{
			std::cerr << "tinyply exception: " << e.what() << std::endl;
		...
		try
		{
			colorsRGB = file.request_properties_from_element("vertex", { "r", "g", "b" });
		}
		catch (const std::exception& e)
		{
			std::cerr << "tinyply exception: " << e.what() << std::endl;
		...
		// Example One: converting to your own application types
		{
			...
			myVerts = std::vector<float3>(vertices->count);
			std::memcpy(myVerts.data(), vertices->buffer.get(), numVerticesBytes);
		}

		{
			const size_t numVerticesBytes = colorsRGB->buffer.size_bytes();
			myColors = std::vector<Color>(colorsRGB->count);
			std::memcpy(myColors.data(), colorsRGB->buffer.get(), numVerticesBytes);
		}
